...
void pcf8574_write(int exp_num, int port_num, int port_state1, int send_cmd){
	...
	if(send_cmd==1){
		...
		tx[0]= txdata;
//		HAL_I2C_Master_Transmit(&hi2c2,addr[0],tx,1,100);
		//HAL_I2C_Master_Transmit_DMA(&hi2c1, addr[0], tx, 1);

		HAL_I2C_Master_Transmit(&hi2c2,pcf8574_addr[exp_num],tx,1,100);
	}
}
